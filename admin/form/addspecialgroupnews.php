...
<body class="hold-transition sidebar-mini">
    ...
    <div class="wrapper">
        ...
        if ($getid == '' || $getid == null) {
            $sqlgroup = "SELECT max(cpa_groupspecial_id)as maxid FROM cpa_web_groupspecial_news";
            ...
            if (isset($_POST['submit'])) {
                ...
                $sqladdgroup = "INSERT INTO cpa_web_groupspecial_news (cpa_groupspecial_id,cpa_groupspecial_name,note,status) VALUE ('$idgroup','$groupnews2','$note','1')";
                ...
                if ($querysqladdgroup) {
                    echo '<script>
                            Swal.fire({
                                ...
                                window.location = "./addspecialgroupnews.php";
                            });
                        </script>';
                } else {
        ...
        if ($getid != '' || $getid != null) {
            $idgroup = $getid;
            $sqlgroup = "SELECT * FROM cpa_web_groupspecial_news where cpa_groupspecial_id = '$getid'";
            $queryGetqroup = mysqli_query($con, $sqlgroup);
            $resultgetgroup = mysqli_fetch_assoc($queryGetqroup);
            $getGroupname =  $resultgetgroup['cpa_groupspecial_name'];
            $getnote =  $resultgetgroup['note'];
            ...
            if (isset($_POST['edit'])) {
                ...
                $sqlEditgroup = "UPDATE cpa_web_groupspecial_news SET cpa_groupspecial_name = '$newsgroupnews', status = '$newstatus', note = '$newnote' where cpa_groupspecial_id = '$getid'";
                $querysqlEditgroup = mysqli_query($con, $sqlEditgroup);

                $sqlInsertlog = "INSERT INTO cpa_web_groupspecial_news_log (cpa_groupspecial_name_old,cpa_groupspecial_name_new,status_old,status_new,cpa_log_id,updateuser) 
                VALUE ('$getGroupname','$newsgroupnews','$getstatus','$newstatus','$getid','$userupdate')";
                ...
                if ($querysqlEditgroup) {
                    ...
                    Swal.fire({
                        ...
                        window.location = "./tableshowdata.php?page=addspecialgroupnews";
                    });
                </script>';
                } else {
            ...
            <div class="container-fluid">
                <div class="card card-info">
                    <div class="card-header">
                        <h3 class="card-title"> เพิ่มกลุ่มหัวข้อย่อยข่าวประชาสัมพันธ์</h3>

                        <?php if ($getid != '' || $getid != null) {
                            $sqlgetfrompage = "SELECT * FROM cpa_web_groupspecial_news_log where cpa_log_id= '$getid' order by updatetime desc limit 15";
                    ...
                    <div class="card-body">
                        ...
                        <form action="#" method="POST">
                            ...
                            <div class="form-group">
                                <input type="text" class="form-control" name="groupnews" value="<?= $getGroupname ?>" placeholder="เช่น ข่าวประชาสัมพันธ์ หัวข้อ A" required>
                            </div>
                            <div class="form-group">
                                <input type="text" class="form-control" name="note" value="<?= $getnote ?>" placeholder="หมายเหตุ " >
                            </div>
                            <?php if ($getid == '' || $getid == null) { ?>
